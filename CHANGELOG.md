...
- SPF: warnings from looking up an `include:` or `redirect=` target's record (a TXT-plus-CNAME conflict, deprecated SPF-type records, oversized records) were dropped, also when that target's record then failed to parse; they are now part of the results next to the parser's warnings for the target, or of the error result when the check fails
