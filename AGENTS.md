...
- **`main` is protected by a ruleset, so changes land through a pull request.**
  The "Protect main" ruleset requires a pull request (no approvals needed) whose
  `CodeQL`, `Lint, type-check, and build checks`, and `Tests (Python 3.10)`
  through `Tests (Python 3.14)` checks all pass, and it blocks deleting or
  force-pushing `main`. Codecov is deliberately not required — an outage on an
  external service should not stop a merge. The maintainer holds a bypass for
  emergencies; nobody else does, and the bypass is an escape hatch rather than
  the normal route. Adding or renaming a CI job that should gate merges means
  updating the ruleset's required-check list too, or the new job passes without
  ever being required.
- **Check for and resolve open security alerts and Dependabot PRs before creating a PR branch.** Before branching for a release or a substantive change, look at all three of the repository's open Dependabot / security alerts (`gh api repos/domainaware/checkdmarc/dependabot/alerts`), open code scanning alerts (`gh api repos/domainaware/checkdmarc/code-scanning/alerts --jq '.[] | select(.state=="open")'`), AND any open Dependabot pull requests (`gh pr list --repo domainaware/checkdmarc --author 'app/dependabot'`), and fold the fixes into the same branch rather than leaving them to pile up against `main`. A Dependabot PR that's already green can simply be merged into the release branch instead of hand-applied; the point is that nothing security-relevant ships a release with the fix still sitting unmerged. Pin the minimum dependency version *above* the first patched version named in the advisory, and verify the new floor actually co-installs with its peers (e.g. `cryptography` and `pyopenssl` constrain each other release-to-release) by doing a clean resolve and running the full test suite against the resolved versions. This is a library, so update the version ranges in `pyproject.toml` and `requirements.txt` — do not add a lock file. Record the fix as a "Security" CHANGELOG entry citing the advisory (e.g. its GHSA id).
  ...
  - A code scanning alert in test code is still an alert. Fix the pattern
    ...
    matches `notexample.com`), so pinning the exact expected text both clears
    the alert and makes the test stronger.
...
- **All code must conform to official RFC or API documentation.** This is a standards-validation tool, so the authoritative spec text — the RFC (or draft) for protocol behavior, the vendor's official documentation for an API — is the contract the code implements, not a summary, a blog post, or memory of the spec. Concretely: fetch and read the governing section (especially the formal grammar/ABNF) before writing or changing validation logic; cite the RFC section in a comment at the decision point; and when a record the spec calls valid would be rejected, or one it calls invalid accepted, that is a bug even if tests pin the current behavior. When a spec's prose and its formal grammar disagree (e.g. RFC 8461's literal `v=STSv1;` discard rule vs. its `*WSP ";" *WSP` field delimiter), accept the grammar-valid form and emit a warning describing how strict prose-following implementations will treat it. And verify every spec claim in a review — human or AI — against the actual document before acting on it: a reviewer once cited an ABNF production that does not exist in the RFC.
- **Enforce a spec limit at the site that counts it, once.** Duplicating a limit check in each branch that increments the counter (as the old SPF code did with the 10-DNS-lookup limit) guarantees the copies drift and some branches skip enforcement. Put the counter and its check in one helper or one post-increment location so every path that counts also enforces.
- **Run checks over the same file set and mode CI uses before pushing.** `pyright` locally must cover `tests/` as well as `checkdmarc/` (CI checks both), and coverage should be verified with `GITHUB_ACTIONS=true` set, because network tests are skipped there: a branch that only live-network tests reach counts as uncovered on CI, and needs a deterministic mocked test (mock as the trigger, assert on behavior).
...
- **Cross-check reviews are a repeatable way to find real bugs.** Run them as separate passes; each pass compares two representations of the same intent, and a disagreement marks a bug in one of them. (1) *Prose vs. code:* review docs, docstrings, comments, and user-facing strings for spelling/grammar/clarity, and check every concrete claim against the code itself — documented parameters, defaults, and return keys vs. the actual signatures and constants, docs usage text vs. real `--help` output, error messages vs. the condition that raises them (a message can state the opposite of its check). (2) *Names vs. values:* find identifiers whose names misdescribe what they hold — variables reassigned to a different type or meaning mid-function, declared fields nothing produces, config knobs wired to the wrong constant, names copy-pasted from a sibling module, the same concept named differently across sibling APIs. (3) *Coverage vs. reachability:* before testing an uncovered line, check that the branch is reachable and that its guard is correct — a nearly-unreachable error branch usually means the validation above it is broken; fix the validation, don't pin broken behavior with a test. In every pass: treat findings as hypotheses and confirm suspected bugs empirically (run code demonstrating the wrong behavior) before fixing; update tests that assert on changed strings in the same change; give renamed public API a deprecated alias that warns.
